...
subscribe_topic, publish_topic = f"emission/{veh_id}/max_co2", f"emission/{veh_id}/max_co2/trigger"

#call back function, subscribe to the desired topic and register a callback.
#print lambda function output
def subscribe(client, userdata, message):
    print(f'{veh_id} subscribed max_co2 data from the subscribed topic {subscribe_topic}:{message.payload}')
...
for row in data:
    ...
    print(f'{veh_id} published vehicle emission data to the topic {publish_topic}: {json_message}\n')
    time.sleep(1)
